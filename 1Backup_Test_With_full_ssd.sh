...
function is_ssd_full() {
    local threshold_percent=99
    local ssd_usage=$(df -h | grep '/dev/sd' | awk '{print $5}' | sed 's/%//')
    if [ "$ssd_usage" -ge "$threshold_percent" ]; then
        return 0  #SSD переповнений
    else
        return 1  #SSD не переповнений
    fi
}
...
function backup_node() {
    ...
    if ! is_ssd_full; then
        printRed "SSD переповнений. Не вдалось виконати бекап для ноди $node_name"
        return
    fi
    ...
    if [ "$backup_message_printed" == true ]; then
        ...
        printRed "Не знайдено файли для бекапу ноди $node_name"
    fi
}
